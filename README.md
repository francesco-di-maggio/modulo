...
Modules can transmit both control (data) and audio signals. In most cases, data is normalized within a bipolar (```-1. - 1.```) or unipolar (```0. - 1.```) range of floating point numbers, simplifying adaptation and communication with subsequent modules. Since each device allows a different set of input channels, each module is designed to automatically adapt to the number of input channels. 
